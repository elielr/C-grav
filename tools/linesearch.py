...
class LineSearch:
    ...
    **So far just an intuition, between a initial speed whose trajectory ends in planet A and another that ends in planet B (or the border)
    ...
    def __init__(self, traj, n=100, v1=None, v2=None):
        ...
        self.traj = traj
        self.eval = np.zeros((n+2,3),dtype='float')
        ...
        if not v2 is None:
            ...
            while self.eval[-1,1] == self.eval[0,1]:
    ...
    # https://en.wikipedia.org/wiki/Line_search doesn't apply here, even 0-order golden-section, our function is absolutely not unimodal
    ...
    def dicho_frontiersearch(self):
        ...
        a0, a1 = 0, self.n+1
        self.eval[1:-1] = np.zeros((self.n,3))
        for i in range(1,min(self.n+1,50)): # 0.5**50 = 1e-15 no need to go much below double precision since position takes values around 1
            self.eval[i,0] = (self.eval[a0,0]+self.eval[a1,0])/2
            self.eval[i,1:] = self.ending(self.v(self.eval[i,0]))
    ...
    def mixed_search(self,n_step=100):
        ...
        for i in range(n_zoom):
            ...
            # select zommed in region of interest
            a2 = np.argmax(mask)
            if a2 == a0:
                a1 = i*n_step+1
            elif a2 == (i+1)*n_step:
    ...
    # dicho_multisearch, if a!=b!=c explore on both sides

    # mixed_multidir_search, reinitialize v0[1] multiple times to look for wider optimums in easier zones

    # mixed_multidir_multisearch
...
class DisplayLineSearch(Display, LineSearch):
    ...
    def plot1D(self,ev=None,ymin=0,ymax=0, ax=None):
        ...
        ax.vlines(self.eval[:,0][self.eval[:,1] == self.traj.N+2], ymin, ymax, colors="#8FE8FF",zorder=1)
        ax.vlines(self.eval[np.argmax(self.eval[:,2]),0], ymin, self.eval[np.argmax(self.eval[:,2]),2], colors='r', zorder=2)
