...
class DisplayVoronoi(Display):
    ...
    def vor_crashsite(self,ax):
        ...
        for i in range(self.sol.n): # no need to add "if not -1 in region :" since those are at n,...,n+4
            ...
            ax.fill(*zip(*polygon),color = self.color_list[int(self.sol.res[i,0])-1])
    ...
    def plot4(self, figsize = None, save = False):
        ...
        if save :
            ...
            np.save('save/montecarlo/[{}, {}].npy'.format(self.traj.p0[0],self.traj.p0[1]),self.sol.points)
            fig.savefig('figs/[{}, {}] - mc({}).png'.format(self.traj.p0[0],self.traj.p0[1],self.sol.n),bbox_inches='tight')